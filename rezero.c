...
static void writestructs(SDF *sdfp, FILE *fp)
{
    ...
    void *btab;
    void **addrs;
    float set_radius, vel=4.5;
    double x, y, z, radius;
    float vx;


    printf("quantity to reset: \n");
    scanf("%s",member);
...
/* up to here just the SDF file is read in, so everything stays the same -CIE */
    ...
    for (j = 0; j < nrecs; ++j) {
        ...
        if( radius >= set_radius) {
            ...
            memcpy(btab + inoffsets[ixvel], &vel, SDFtype_sizes[ types[ixvel] ]);

            if( aflag ) {
                /* replace quantity - ay*/
                memcpy(btab + inoffsets[ixvel+1], &vel, SDFtype_sizes[ types[ixvel] ]);
    
                /* replace quantity - az */
                memcpy(btab + inoffsets[ixvel+2], &vel, SDFtype_sizes[ types[ixvel] ]);
            }
