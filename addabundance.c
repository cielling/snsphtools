...
/*one by one, go through all the fields in the column, i.e. members of the struct?-CE*/
	...
	if (flag) {
	    ...
	    nobjs[nmembers] = nrecs;/*nobjs[0] is the number of particles. are all elements
    ...
    printf("about to malloc %u bytes for btab\n", nrecs*stride);

    btab = (void *)malloc(nrecs * stride);
...
/* up to here just the SDF file is read in, so everything stays the same -CE */
    ...
    outbtab = (void *)malloc( nrecs*outstride );


    /* copy btab data to output 'array' -CE */
    for (j = 0; j < nobjs[0]; ++j) {
	for (i = 0; i < nmembers; ++i) {
               memcpy(outbtab + j*outstride + inoffsets[i],
	       btab + j*stride + inoffsets[i],
	       SDFtype_sizes[ types[i] ]);
	}
    }
    free(btab); /* we're done with btab; it's all in outbtab now */
    ...
    for (j = 0; j < nobjs[0]; ++j) {
        ...
        x = *(double *)(outbtab + j*outstride + inoffsets[0]);
        y = *(double *)(outbtab + j*outstride + inoffsets[1]);
        z = *(double *)(outbtab + j*outstride + inoffsets[2]);
...
/* where is the pointer when fwrite is done? can I repeat the fwrite
   right after this and it will automatically append the content? -CE */
